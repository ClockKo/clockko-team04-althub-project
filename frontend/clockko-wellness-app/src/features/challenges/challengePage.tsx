...
async function fetchChallengeStats(): Promise<ChallengeStats> {
  const res = await fetch("/api/challenges/stats", { credentials: "include" });
  if (!res.ok) throw new Error("Failed to fetch challenge stats");
  return await res.json();
}

async function fetchChallenges(): Promise<Challenge[]> {
  const res = await fetch("/api/challenges/week", { credentials: "include" });
  if (!res.ok) throw new Error("Failed to fetch challenges");
  return await res.json();
}

async function fetchLeaders(): Promise<Leader[]> {
  const res = await fetch("/api/challenges/leaders", { credentials: "include" });
  if (!res.ok) throw new Error("Failed to fetch leaders");
  return await res.json();
}

async function startChallenge(challengeId: string): Promise<Challenge> {
    ...
    >
      ...
      {isInProgress && challenge.progress !== undefined && challenge.total !== undefined && (
        <div className="mb-2">
          ...
          <div className="text-xs text-right mt-1">{challenge.progress}/{challenge.total}</div>
        </div>
      )}
      <div>
        {isCompleted ? (
          <Button disabled variant="outline" className="w-full">Completed</Button>
        ) : isInProgress ? (
          <Button disabled variant="outline" className="w-full">In Progress</Button>
        ) : (
        ...
        {leaders.map((l) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className={cn("font-medium", l.isCurrentUser && "text-indigo-700")}>{l.name}</span>
            </div>
    ...
    <div className="min-h-screen w-screen bg-gray-50 md:px-2 xs:px-4 py-2 lg:w-[96vw] max-w-[1440px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {leadersLoading ? (
          ...
          <Leaderboard leaders={displayLeaders} />
        )}
      </div>
    </div>
  );
