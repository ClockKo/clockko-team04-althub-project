...
export function WelcomeModal({ onNext }: { onNext?: () => void }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
    >
      <div className="bg-coolWhite rounded-2xl p-4 xs:p-6 w-full max-w-xs xs:max-w-md md:max-w-[600px] md:h-[400px] mx-auto text-center shadow-lg relative">
        <img src={Poses3} alt="koala bear poses" className="w-16 mx-auto mb-4 mt-14" />
        <h2 className="text-dark text-xl xs:text-2xl font-bold mb-2">Welcome to ClockKo!</h2>
        <p className="text-darkGray mb-6 text-sm xs:text-base font-thin md:w-[400px] mx-auto">
